...
class OptimalExecutionEnv:
    def __init__(self, initial_shares: float, total_time_steps: int, lookback_window: int = 5):
        ...
        # Account for 5-minute stepping: need T*5 data points per episode
        ...
        # Track cumulative metrics for terminal reward
        self.cumulative_revenue = 0.0
        self.cumulative_slippage_sq = 0.0
    ...
    def step(self, state: OptimalExecutionState, action: Action) -> Tuple[OptimalExecutionState, float, bool, Dict]:
        ...
        # 6. Calculate Risk-Averse Reward (Terminal Reward Only!)
        # Accumulate metrics, but don't give reward until episode ends
        self.cumulative_revenue += Revenue_t
        relative_slippage = (P_exec - self.P_REF) / self.P_REF
        self.cumulative_slippage_sq += (relative_slippage ** 2)
        
        # No intermediate rewards - encourages long-horizon planning
        R_t = 0.0
        ...
        # TERMINAL REWARD: Give reward only at episode end
        if done:
            # Total revenue achieved
            total_revenue = self.cumulative_revenue
            
            # Total risk penalty (variance of execution prices)
            total_risk = self.LAMBDA * self.cumulative_slippage_sq * (self.P_REF ** 2)
            
            # Implementation shortfall: Compare to "sell all at P_REF" baseline
            benchmark_revenue = self.X0 * self.P_REF
            shortfall = benchmark_revenue - (total_revenue - total_risk)
            
            # Reward = negative shortfall (want to minimize shortfall)
            # Normalize AND scale by 10000 to make learning signal stronger
            # This gives range approximately [-10, 10] instead of [-0.01, 0.01]
            R_t = (-shortfall / benchmark_revenue) * 10000.0
            
            # Penalize heavily if didn't complete liquidation
            if X_next > 1e-6:
                R_t -= 100.0  # Big penalty for leaving inventory

        # 8. Update State Vector (S_{t+1})
        self.current_step += 1

        # Fetch the next price trend for the augmented belief state
        # Since DELTA_T=5, each step represents 5 minutes, so skip 5 data points
        # This aligns with the noise scaling: sigma * sqrt(5)
